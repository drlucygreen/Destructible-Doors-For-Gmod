...
if SERVER then 
	...
	CreateConVar("db_doorhealth", 900, {FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE, FCVAR_NOTIFY},"How strong the doors are.")
	...
	CreateConVar("db_lockopen", 1, {FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE, FCVAR_NOTIFY},"Wether or not doors should be opened and unlocked after being shot open.")
	...
	hook.Add("EntityTakeDamage","BigBadWolfIsJealous", function(prop, dmginfo)
		...
		if (prop:GetClass() == "prop_door_rotating" and IsValid(prop) )then
			...
			--print("Took a total of " .. dmgtaken .. " damage")

			--print("door health was" .. doorhealth)
			...
			if prop:Health() <= 0 and !prop.bIsBusted then -- Makes sure that the door doesn't create props until after it's respawned
	...
	hook.Add("PlayerSay", "DoorsAndDoorAccessories", function(ply, text) -- Chat commands to make life easier
		...
		if ( string.sub(text, 1, 11) == "!resetdoors" ) then
